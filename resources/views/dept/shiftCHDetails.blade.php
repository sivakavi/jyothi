    	...
    	$(function () {
    	   	$('.saveModal').click(function(){
                if($('.emp_status').val() !="" && $('.emp_work_type').val() !="")
                {
                    ...
                    ajaxURL = "{{route('dept.shiftCHDetailsChange')}}";
                    if(assignShiftId == ""  && $('#bulk_change').val() == 0){
                        assignShiftId = 0;
                        ajaxURL = "{{route('dept.employeeCHAdd')}}";
                    }
                    else if($('#bulk_change').val() == 1){
                        ajaxURL = "{{route('dept.shiftCHBulkDetailsChange')}}";
                    }
                    empDate = $('#shiftDate').val();
                    empId = $('#emp_id').val();
                    shift_id = $('#shiftID').val();
			...
			$(document).on('click', '#empbulkassign', function(e) {
    	   	...
    	   	$('.empSearch').click(function(e){
                ...
                department_id = $('#department_id').val();
    	   		...
    	   		if(empName != ''){
    	   			jQuery.ajax({
						url: "{{route('dept.employeeCHSearch')}}",
						type: 'GET',
						data: datas,
						success:function(data) {
