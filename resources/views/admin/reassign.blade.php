...
@section('content')
    ...
    <div class="row margin-top-30">
        <div class="col-md-8 center-margin">
            <form class="form-horizontal form-label-left" action="{{ route('admin.employeeReassignStore') }}" method="POST">
                            ...
                            <div class="x_panel">
                                ...
                                <div class="x_content margin-top-40">
                                    ...
                                    <div class="form-group @if($errors->has('department_id')) has-error @endif">
                                        ...
                                        <select id = "shift_id" class="form-control emp_shift" name="shift_id">
                                            @foreach($shifts as $shift)
                                                <option value="{{$shift->id}}" @if(($batches['shift_id'] == $shift->id) || ($batches['changed_shift_id'] == $shift->id)) selected @endif>{{$shift->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    ...
                                    <div class="well well-sm margin-top-50">
                                        ...
                                        <a class="btn btn-link pull-right" href="{{ route('admin.employeeReassignList') }}"><i class="glyphicon glyphicon-backward"></i> Back</a>
                                    </div>
    ...
    <script type="text/javascript">
        $(function () {
            ...
            $( "#department_id" ).change(function() {
                var ajaxUrl = "{{ route('admin.getShift') }}";
                $.ajax({
                    url: ajaxUrl,
                    type: 'GET',
                    data: {
                ...
                var ajaxUrl = "{{ route('admin.getStatus') }}";
                $.ajax({
                    url: ajaxUrl,
                    type: 'GET',
                    data: {
                ...
                var ajaxUrl = "{{ route('admin.getWorkType') }}";
                $.ajax({
                    url: ajaxUrl,
                    type: 'GET',
                    data: {
